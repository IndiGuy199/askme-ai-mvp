 ...
 * Phase 6 rewrite: opportunity-based weighted scoring (JS version for testing)
 *
 * Weights:
...
function existsDuringRange(createdAt, removedAt, startDate, endDate) {
...
function computeGoalsActiveInRange(events, goals, rangeStart, rangeEnd) {
  ...
  const toSlotKey = (slot) => String(slot ?? '__unknown__');
  ...
  const openGoal = (goalId, slotKey, start) => {
  ...
  const closeGoal = (goalId, closeAt) => {
...
function computeReportCompleteness(metrics) {
  ...
  if (actionsPlanned === 0) {
    ...
    if (completionQualityAvg !== null && completionQualityAvg !== undefined) {
  ...
  const toolSamplesByCategory = (metrics.tools?.best_categories || []).map((categoryRow) => ({
...
function getToolConfidence(sampleSize) {
  if (sampleSize >= 10) return 'high';
  if (sampleSize >= 5) return 'medium';
  return 'low';
}

function formatRiskWindowLabel(topHours) {
...
function mapCategoryToDescription(category) {
  const descriptions = {
...
function mapConfidenceToCopy(sampleSize, itemType = 'logs') {
...
function mapRiskConfidence(totalLogsWithTimestamps) {
  if (totalLogsWithTimestamps >= 25) return 'high';
  if (totalLogsWithTimestamps >= 10) return 'medium';
  return 'low';
}

function shouldRenderHeatmap(totalEventsInRange, minEvents = 10) {
  return totalEventsInRange >= minEvents;
}

function shouldRenderStrategies(totalUrgeRatingsInRange, minEvents = 5) {
  return totalUrgeRatingsInRange >= minEvents;
}

module.exports = { 
  existsDuringRange,
  computeGoalsActiveInRange,
